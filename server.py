...
if not st.session_state.registado:
    ...
    st.markdown("[📸 Seguir no Instagram](https://www.instagram.com/seu_usuario)") # COLOQUE SEU LINK AQUI
    st.markdown("[🎵 Seguir no TikTok](https://www.tiktok.com/@seu_usuario)")      # COLOQUE SEU LINK AQUI
    
    st.write("---")
    nome = st.text_input("Nome:")
    check_social = st.checkbox("Confirmo que segui o Grupo FF no Instagram e TikTok")
    ...
    if st.button("Concluir Registo"):
        if nome and check_social:
            ...
            st.error("Por favor, digite seu nome e confirme que nos seguiu nas redes sociais!")
    ...
    with col_main:
        ...
        if 'resultados' in st.session_state and st.session_state.resultados:
            ...
            if st.button("Confirmar Pedido"):
                ...
                del st.session_state['select_musica']
                st.session_state.input_busca = ""
                st.rerun()
        
        # PEDIDO MANUAL
        st.markdown("<br><label>Título/Cantor que não estão na nossa lista:</label>", unsafe_allow_html=True)
        ...
        if st.button("Enviar Pedido Manual"):
            if manual:
                ...
                st.markdown('<div class="warning-box">Pedido enviado, mas sujeito a disponibilidade.</div>', unsafe_allow_html=True)
